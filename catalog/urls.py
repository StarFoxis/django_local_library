...
from . import views
# from django.conf.urls import url - устарел, используй re_path

urlpatterns = [
    re_path('^$', views.index, name='index'),
    re_path(r'^books/$', views.BookListView.as_view(), name='books'),
    re_path(r'^book/(?P<pk>\d+)/$', views.BookDetailView.as_view(), name='book-detail'),
    re_path(r'books/year/(?P<year>\d{4})/$', views.BookYearListView.as_view(), name='books-year-list'),
    re_path(r'^authors/$', views.AuthorListView.as_view(), name='authors'),
    re_path(r'^author/(?P<pk>\d+)/$', views.AuthorDetailView.as_view(), name='author-detail'),
    re_path(r'^mybooks/$', views.LoanedBooksByUserListView.as_view(), name='my-borrowed'),
    re_path(r'^allbooks/$', views.LoanedBooksByUsersListView.as_view(), name='all-borrowed'),
